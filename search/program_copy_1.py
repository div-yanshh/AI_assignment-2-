...
def find_jump_sequences(start: Coord, board: dict[Coord, CellState], path=None, first_call=True) -> list[list[Direction]]:
    """
    Recursively finds all possible jump sequences starting from 'start' on 'board'.
    The 'path' list tracks positions visited in the current jump chain to prevent cycles.
    The parameter first_call indicates whether this is the first jump in the chain.
    """
    if path is None:
        path = []
    new_path = path + [start]
    ...
    for direction in allowed_directions:
        ...
        if not (0 <= jump_r < BOARD_N and 0 <= jump_c < BOARD_N):
            continue
        jump_dest = Coord(jump_r, jump_c)
        # Must have an unoccupied lily pad.
        if jump_dest not in board or board[jump_dest] != CellState.LILY_PAD:
            continue
        ...
        # Simulate the jump.
        # If this is the first jump in the chain, represent it as a single direction.
        if first_call:
        ...
        # If this jump reaches the goal, then we mark this branch as complete.
        if goal_test(new_board):
            # Return a complete jump chain as the current jump only.
            # The parent's recursion will combine it with its own move.
            jump_sequences.append([direction])
            continue
        
        # Otherwise, recursively find further jump sequences from jump_dest.
        subsequent_sequences = find_jump_sequences(jump_dest, new_board, new_path, first_call=False)
        if subsequent_sequences:
            for seq in subsequent_sequences:
                jump_sequences.append([direction] + seq)
        else:
